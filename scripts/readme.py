...
class TableInform:
    ...
    def __create_folder(self, oj_name):
        ...
        for item in self.table_item.values():
            question_folder_name = oj_algorithms + '/' + str(int(item.id_)) + '_' + item.title.replace(" ", "")
            ...
            if not os.path.exists(question_folder_name):
                print(question_folder_name + ' does not exits, create it now....')
                ...
                for _, _, files in os.walk(os.path.join(oj_algorithms, folder)):
                    ...
                    for item in files:
                        ...
                        if item.endswith('.py'):
                            ...
                            # print(folder_url)
                            self.table_item['{0:03}'.format(int(folder.split('_')[0]))].python = '[Python]({})'.format(folder_url)
                            ...
                            folder_url = os.path.join(Config.github_leetcode_url, folder_url)
                            # print(folder_url)
                            self.table_item['{0:03}'.format(int(folder.split('_')[0]))].c_plus_plus = '[C++]({})'.format(folder_url)
                            ...
                            folder_url = os.path.join(Config.github_leetcode_url, folder_url)
                            # print(folder_url)
                            self.table_item['{0:03}'.format(int(folder.split('_')[0]))].javascript = '[JavaScript]({})'.format(folder_url)
